<div class="form-field-editor">
    ...
    <div class="form-field-editor__modal">
        ...
        <div class="form-field-editor__body">
            ...
            <div v-if="hasOptions" class="form-field-editor__field">
                ...
                <div class="form-field-editor__options">
                    ...
                    <button type="button" class="btn btn-sm btn-secondary" @click="addOption">+ <?php \MapasCulturais\i::_e('Adicionar opção') ?></button>
                </div>
            </div>
            ...
            <div v-if="grupos && grupos.length > 0" class="form-field-editor__field">
                <label><?php \MapasCulturais\i::_e('Grupo') ?></label>
                <select v-model.number="edit.grupo_id" class="input-select">
                    <option v-for="g in grupos" :key="g.id" :value="g.id">{{ g.titulo }}</option>
                </select>
            </div>

            <div class="form-field-editor__field">
                <label><?php \MapasCulturais\i::_e('Largura') ?></label>
        ...
        <div class="form-field-editor__actions">
            <button type="button" class="btn btn-secondary" @click="$emit('cancel')"><?php \MapasCulturais\i::_e('Cancelar') ?></button>
            <button type="button" class="btn btn-primary" @click="save"><?php \MapasCulturais\i::_e('Salvar') ?></button>
        </div>
    </div>
</div>
